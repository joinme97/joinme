...
public class SelectedActivityListDAO {
	...
	public boolean userIDANDActivityIDExist(int user_id,int activity_id) {
		...
		try {
			...
			String query = "select * from selected_activity_list where user_id=" + user_id+"&& activity_id="+activity_id;
			ps = conn.prepareStatement(query);
			rs = ps.executeQuery();
			if (rs.next()) {
				flag=true;
	...
	public int getAllSelectedActivityListCountWithActivityID(int activity_id) {
		...
		try {
			...
			String query = "select count(activity_id) as count from selected_activity_list where activity_id="+activity_id;
			ps = conn.prepareStatement(query);
			rs = ps.executeQuery();
			while (rs.next()) {
